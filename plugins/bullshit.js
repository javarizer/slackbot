...
var bullshit = Promise.method(function(data, userData, bot) {
	bot.sendMessage({
		username: "The Corporate B.S. Generator",
		icon_url: "http://100-pics.net/images/answers/en/ilove1990s/ilove1990s_22215_153699.jpeg",
		text: `${adverbs[Math.floor(adverbs.length*Math.random())]} ${verbs[Math.floor(verbs.length*Math.random())]} ${adjectives[Math.floor(adjectives.length*Math.random())]} ${nouns[Math.floor(nouns.length*Math.random())]}`
	});
});

exports.load = function(registry) {
	registry.respond(
		new RegExp("(?:bs|bullshit)$",'im'),
		bullshit
	);
